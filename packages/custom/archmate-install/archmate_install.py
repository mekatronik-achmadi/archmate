...
class ArchMateInstall(archmate_gui.frmArchInstall):
    ...
    def CheckPartSize(self):
        ...
        if self.m_chkEFI.IsChecked():
            ...
            if sizeFsEFI < 500:
                wx.MessageBox("EFI size less than 500MB","Error",wx.OK|wx.ICON_ERROR)
                return False
    ...
    def InstallProc(self):
        ...
        sed_args  = 's@'
        sed_args += 'base udev modconf memdisk archiso_shutdown archiso archiso_loop_mnt consolefont archiso_kms block filesystems keyboard'
        sed_args += '@'
        sed_args += 'base udev autodetect modconf memdisk consolefont block filesystems keyboard fsck'
        sed_args += '@g'
